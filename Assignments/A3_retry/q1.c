// AIM: for a given array, find the longest subsequence of consecutive increasing numbers
...
int main()
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        {
            if(h->table[j]->lowest_value <=x && h->table[j]->final_value >= x)
            {
                makenew = 0;
                continue;
            }
            
            else if(h->table[j]->final_value == x-1)
            {
                makenew = 0;
                insert_to_ll(x, i, j, h);
                if(h->table[j]->count > h->table[index_of_maxcount]->count)
                {
                    index_of_maxcount = j;
                }
                break;
